...
public struct JJVal: CustomDebugStringConvertible {
    ...
    public var asNSNumber: NSNumber? { return _v as? NSNumber }
    
    public func toNSNumber(@autoclosure defaultValue:  () -> NSNumber = 0) -> NSNumber {
        return asNSNumber ?? defaultValue()
    }
    
    public func nsNumber() throws -> NSNumber {
        if let x = asNSNumber {
            return x
        }
    ...
    public func at(key: String) throws -> JJVal {
        return try obj().at(key)
    }
    ...
    public func at(index: Int) throws -> JJVal {
        return try arr().at(index)
    }
